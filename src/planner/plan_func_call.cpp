...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" || func_name == "upper") {
    if (args.size() != 1) {
      throw Exception(fmt::format("Function {} expects exactly one argument", func_name));
    }
    StringExpressionType expr_type = (func_name == "upper") ? StringExpressionType::Upper : StringExpressionType::Lower;
